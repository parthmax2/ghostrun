"""`ghostrun init` -- scaffolds a working first test in one command."""
...
from ghostrun.scaffold import detect_sdk, render_config, render_test_file
...
def test_rendered_test_file_is_valid_python(sdk):
    code = render_test_file(sdk, "test_ghostrun_example.py")
    ast.parse(code)  # raises SyntaxError if the template is broken
    assert "@ghostrun.record" in code
    assert "ghostrun.expect(" in code
...
def test_init_creates_config_and_test_file(tmp_path, capsys):
    ...
    assert rc == 0
    assert (tmp_path / ".ghostrun.yaml").is_file()
    assert (tmp_path / "test_ghostrun_example.py").is_file()
...
def test_init_force_overwrites(tmp_path):
    main(["init", "--dir", str(tmp_path)])
    (tmp_path / "test_ghostrun_example.py").write_text("# modified", encoding="utf-8")
    rc = main(["init", "--dir", str(tmp_path), "--force"])
    assert rc == 0
    assert "# modified" not in (tmp_path / "test_ghostrun_example.py").read_text()
...
def test_init_custom_filename(tmp_path):
    ...
    assert not (tmp_path / "test_ghostrun_example.py").exists()


def test_init_echo_judge_config(tmp_path):
    main(["init", "--dir", str(tmp_path), "--judge", "echo"])
    import yaml
    data = yaml.safe_load((tmp_path / ".ghostrun.yaml").read_text(encoding="utf-8"))
    assert data["judge"]["type"] == "echo"
...
def test_init_generated_file_is_collectible_by_pytest(tmp_path):
    ...
    main(["init", "--dir", str(tmp_path), "--judge", "echo"])
    result = pytest.main(["--collect-only", "-q", str(tmp_path / "test_ghostrun_example.py")])
