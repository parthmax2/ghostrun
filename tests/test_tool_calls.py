...
def test_normalizes_provider_shapes(calls):
    gentest.expect_tool_calls(calls).called("search_orders") \
...
def test_called_failure_lists_actual():
    with pytest.raises(SemanticAssertionError, match="issue_refund"):
        gentest.expect_tool_calls(OPENAI).called("issue_refund")


def test_did_not_call():
    gentest.expect_tool_calls(OPENAI).did_not_call("issue_refund")
    with pytest.raises(SemanticAssertionError):
        gentest.expect_tool_calls(OPENAI).did_not_call("search_orders")


def test_called_once():
    gentest.expect_tool_calls(PLAIN).called_once("search_orders")
    twice = PLAIN + PLAIN
    with pytest.raises(SemanticAssertionError, match="exactly once"):
        gentest.expect_tool_calls(twice).called_once("search_orders")


def test_call_count():
    gentest.expect_tool_calls(PLAIN).call_count(1)
    gentest.expect_tool_calls([]).call_count(0)
    with pytest.raises(SemanticAssertionError):
        gentest.expect_tool_calls(PLAIN).call_count(2)


def test_called_with_is_subset_match():
    calls = [{"name": "search", "arguments": {"q": "x", "limit": 10}}]
    gentest.expect_tool_calls(calls).called_with("search", q="x")  # ignores limit
    with pytest.raises(SemanticAssertionError):
        gentest.expect_tool_calls(calls).called_with("search", q="wrong")


def test_called_with_scans_all_matching_calls():
    calls = [{"name": "search", "arguments": {"q": "a"}},
             {"name": "search", "arguments": {"q": "b"}}]
    gentest.expect_tool_calls(calls).called_with("search", q="b")


def test_none_and_empty_are_zero_calls():
    assert len(gentest.expect_tool_calls(None)) == 0
    gentest.expect_tool_calls(None).did_not_call("anything").call_count(0)


def test_unparsable_arguments_do_not_crash():
    calls = [{"function": {"name": "f", "arguments": "not json{"}}]
    gentest.expect_tool_calls(calls).called("f")  # must not raise


def test_chaining():
    gentest.expect_tool_calls(OPENAI) \
