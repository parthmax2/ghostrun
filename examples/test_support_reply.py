"""Example ghostrun test — runs fully offline against a pre-recorded cache.

The `.ghostrun_cache/` directory next to this file already contains a recorded
...
# and ghostrun uses .ghostrun_cache at the project root.
...
def test_refund_reply_is_helpful_and_empathetic():
    ...
    ghostrun.expect(reply).contains_intent("refund")
    ghostrun.expect(reply).does_not_contain_intent("blaming the customer")
